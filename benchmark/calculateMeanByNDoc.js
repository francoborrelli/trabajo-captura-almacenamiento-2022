...
const smallOcurrencesWords = words.filter((w) => w.ndoc > 10 && w.ndoc <= 1000).map((w) => w.word);

const mediumOcurrencesWords = words
  .filter((w) => w.ndoc > 1000 && w.ndoc <= 3000)
  .map((w) => w.word);

const largeOcurrencesWords = words
  .filter((w) => w.ndoc > 3000 && w.ndoc <= 7000)
  .map((w) => w.word);

const bigOcurrencesWords = words.filter((w) => w.ndoc > 7000).map((w) => w.word);
